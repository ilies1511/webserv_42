...
void	Connection::get_file(void)
{
	if (request.readFile && !(this->_current_response.FileData))
	{
		...
		if (prepare_fdFile()) {
			...
			// assemble_response();
			// _state = State::ASSEMBLE; //Nach wie READ_FILE State allerings mit anderem Status Code, sprich ERROR - RESPONse
